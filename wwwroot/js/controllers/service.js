...
function jenisChange() {
  var jeniskegiatanusaha =
    document.getElementById("jeniskegiatanusaha").ej2_instances[0];
  var klasifikasiusaha =
    document.getElementById("klasifikasiusaha").ej2_instances[0];

  var tempQuery = new ej.data.Query().where(
    "JenisKegiatanUsaha",
    "equal",
    jeniskegiatanusaha.value
  );
...
function subklasifikasiChange() {
  var subklasifikasiusaha = document.getElementById("subklasifikasiusaha")
    .ej2_instances[0];
...
function ratioChange() {
  ...
  if (games.value == "Gross Profit Mark Up") {
    ...
    games.value == "Net Cost Plus Method" ||
    games.value == "Net Cost Plus Method"
  ) {
    text.innerHTML =
      "Net Cost Plus Methode (NCPM) : Operating Profit / (Cost Plus Methode + Operating Expense)";
    document.getElementById("penjelasan").classList.remove("hidden");
  } else if (games.value == "Gross Profit") {
    text.innerHTML = "Gross Profit : Gross Profit / Operating Revenue";
    document.getElementById("penjelasan").classList.remove("hidden");
  } else if (
    games.value == "Return On Sales" ||
    games.value == "Return on Sales"
  ) {
    text.innerHTML =
...
function resetAll() {
  ...
  var hargapokokpenjualan = document.getElementById("hargapokokpenjualan")
    .ej2_instances[0];
  var bebanoperasional =
    document.getElementById("bebanoperasional").ej2_instances[0];
  var labakotor = document.getElementById("labakotor").ej2_instances[0];
  var labaoperasional =
    document.getElementById("labaoperasional").ej2_instances[0];
...
function loadCustomers() {
  ...
  var klasifikasi =
    document.getElementById("klasifikasiusaha").ej2_instances[0];
  var subklasifikasi = document.getElementById("subklasifikasiusaha")
    .ej2_instances[0];
  var jenis = document.getElementById("jeniskegiatanusaha").ej2_instances[0];
  ...
  var bebanoperasional =
    document.getElementById("bebanoperasional").ej2_instances[0];
  var namaperusahaan =
    document.getElementById("namaperusahaan").ej2_instances[0].value;
  ...
  labaoperasional.value =
    penjualan.value - hargapokokpenjualan.value - bebanoperasional.value;
  ...
  if (
      ...
      .then((data) => {
        ...
        if (data.length != 0) {
          ...
          $.each(header, function (key, value) {
            $("#grid-header").append(
              `<th ${
                key == "Nama Perusahaan" || key == "Negara" ? 'width="40%"' : ""
              }>${key}</th>`
            );
          });
          data.forEach((items) => {
    ...
    fetch(
      ...
      .then((data) => {
        ...
        // let header = data[0];
        // let header = data?.[0] ?? null; // kalau data atau data[0] undefined → null
        // console.log("Header:", header);
        // $.each(header, function (key, value) {
        //   $("#grid2-header").append(
        //     `<th ${key == "Keterangan" ? 'width="80%"' : ""}>${key}</th>`
        //   );
        // });
        let header = data && data.length > 0 ? data[0] : null;
        console.log("Header:", header);
        ...
        if (header) {
          ...
          // Jika data null, tampilkan modal
    ...
    if (
      !penjualan.value ||
      !hargapokokpenjualan.value ||
      !bebanoperasional.value
    ) {
      testedparty.value = 0;
    } else if (
      rasio.value == "Gross Profit" ||
      rasio.value == "Resale Price Method"
    ) {
      testedparty.value =
        (penjualan.value - hargapokokpenjualan.value) / penjualan.value;
      testedparty.value = Math.round(testedparty.value * 100) / 100;
    } else if (
      rasio.value == "Gross Profit Mark Up" ||
      rasio.value == "Cost Plus Method"
    ) {
      testedparty.value =
        (penjualan.value - hargapokokpenjualan.value) /
        hargapokokpenjualan.value;
      testedparty.value = Math.round(testedparty.value * 100) / 100;
    } else if (
      rasio.value == "Net Cost Plus Method" ||
      rasio.value == "Net Cost Plus Method"
    ) {
      testedparty.value =
        (penjualan.value - hargapokokpenjualan.value - bebanoperasional.value) /
        (hargapokokpenjualan.value + bebanoperasional.value);
      testedparty.value = Math.round(testedparty.value * 100) / 100;
    ...
    document.getElementById("klasifikasiusaharesult").innerHTML =
      klasifikasi.value;
    document.getElementById("subklasifikasiusaharesult").innerHTML =
      subklasifikasi.value;
    document.getElementById("ratioresult").innerHTML = rasio.value;
    document.getElementById("testedpartyresult").innerHTML =
      testedparty.value + " %";
